...
import { sourcedSlugs } from "@/lib/affiliate";
...
export function GET() {
  ...
  if (sourced.length) {
    out.push(
      "",
      "## Availability",
      `- [What the network sources](${SITE}/available): the molecules in this catalog you can reach at research grade, sourced through the American Peptide network via AminoClub, ElyriaBio and Paramount Peptides, research-use-only. Availability is disclosed data, not a storefront — the site sells nothing. Per-vendor pages at /available/aminoclub, /available/elyriabio and /available/paramount list what each carries, with product links and the reader code where one exists.`,
    );
